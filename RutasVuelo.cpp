...
void procesarArchivoCiudades(string nombreArchivo){
	...
	if(!archivo.is_open()){
		cout << "No se logro abrir el archivo de ciudades.\n";
		return;
	}
	
	cout << "\nProcesando el archivo de las ciudades..." << endl;
	vector<string> fila;
...
void procesarArchivoVuelos(string nombreArchivo){
	...
	rutas.imprimir();
	cout << endl; //Esto hay que arreglarlo
}
...
void procesarSolicitudVuelo(){
	...
	if(codigoCiudades.count(ciudadDestino)==0){
		cout << "Lo siento, no volamos haciaa esa ciudad." << endl;
		return;
	}
